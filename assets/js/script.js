...
function displayTrendCoins(coins) {
	...
	coins.forEach(coin => {
		...
		row.innerHTML = `
					<td class="name-column table-fixed-column"><img src="${coinData.thumb}" alt="${
			coinData.name
		}"> ${coinData.name} <span>(${coinData.symbol.toUpperCase()})</span></td>
...
function displayAssets(data) {
	...
	data.forEach(asset => {
		...
		row.innerHTML = `
					...
					<td class="name-column table-fixed-column"><img src="${asset.image}" alt="${
			asset.name
...
function displayExchanges(data) {
	...
	data.forEach(exchange => {
		...
		row.innerHTML = `
					...
					<td class="name-column table-fixed-column"><img src="${exchange.image}" alt="${
			exchange.name
		}"> ${exchange.name}</td>
...
function displayCategories(data) {
	...
	data.forEach(category => {
		...
		row.innerHTML = `
					...
					<td class="${category.market_cap_change_24h >= 0 ? 'green' : 'red'}">${
	...
	data.companies.forEach(company => {
		...
		row.innerHTML = `
				 <td class="name-column table-fixed-column">${company.name}</td>
					...
					<td class="${company.percentage_of_total_supply >= 0 ? 'green' : 'red'}">${
